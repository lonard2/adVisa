...
struct PDFPreviewVisa: View {
    ...
    var body: some View {
        ...
        .onAppear(perform: loadAndFillPDF)
    }
    
    func fillPDFFormFields(pdfDocument: PDFDocument, surname: String, givenName: String, middleName: String) -> PDFDocument? {
            ...
            for annotation in page.annotations {
                if let fieldName = annotation.fieldName {
                    ...
                    case "topmostSubform[0].Page1[0].T0[1]":
                        annotation.widgetStringValue = pdfPreviewViewModel.domicileData?.currentAddress // current resident address
    ...
    func loadAndFillPDF() {
        if let pdfURL = Bundle.main.url(forResource: "visa", withExtension: "pdf"),
           let document = PDFDocument(url: pdfURL) {
            let filledDocument = fillPDFFormFields(pdfDocument: document, surname: "Doe", givenName: "John", middleName: "M")
            self.pdfDocument = filledDocument
        }
    }
}
